...
android {
    namespace = "dev.heckr.court"
    compileSdk = 36

    defaultConfig {
        applicationId = "dev.heckr.court"
        minSdk = 31
        targetSdk = 36
...
androidComponents {
    onVariants { variant ->
        ...
        val apkFileName = when (variant.buildType) {
            "debug" -> "court-dev-${versionName.replace(Regex("-.*"), "").replace(".", "-")}.apk"
            "release" -> "court-release-${versionName.replace(".", "-")}.apk"
            else -> "court-${variant.name}.apk"
        }
